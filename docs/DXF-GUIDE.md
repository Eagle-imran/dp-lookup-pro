...
| `C-PROP-HATCH` | Shading inside the boundary | Visual only. Turn it off when tracing. |
...
| `C-RESTRICT-ZONE` | CRZ or Metro restriction notes | **Read this before massing.** CRZ caps what you can build. |
...
| `C-ANNO-TEXT` | Just the CTS number at the centre, plus the UTM tie-in above the plot | Reference. Full metadata is in the PLOT DATA panel. |
...
you'll see a note saying so on the layer. That's deliberate: better a missing
line than a wrong one.
